...
describe '[STEP3] 仕上げのテスト' do
  ...
  describe 'サクセスメッセージのテスト' do
    ...
    it 'ユーザ新規登録成功時' do
      ...
      expect(find('.hidden-message', visible: false).text(:all)).to include 'successfully'
    end
    it 'ユーザログイン成功時' do
  ...
  describe '処理失敗時のテスト' do
    context 'ユーザ新規登録失敗: nameを1文字にする' do
      ...
      it 'バリデーションエラーが表示される' do
        click_button 'Sign up'
        expect(find('.hidden-message', visible: false).text(:all)).to include "is too short (minimum is 2 characters)"
      end
    end
    ...
    context '投稿データの新規投稿失敗: 投稿一覧画面から行い、titleを空にする' do
      ...
      it 'バリデーションエラーが表示される' do
        click_button 'Create Book'
        expect(find('.hidden-message', visible: false).text(:all)).to include "can't be blank"
      end
    end
    ...
    context '投稿データの更新失敗: titleを空にする' do
      ...
      it 'エラーメッセージが表示される' do
        expect(find('.hidden-message', visible: false).text(:all)).to include 'error'
      end
    end
  end
